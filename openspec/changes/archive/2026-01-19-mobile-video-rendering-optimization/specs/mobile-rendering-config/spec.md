...
`detectCompositionId` 函数 SHALL 默认返回 `HagicodeReleaseNotesMobile`。
...
此变更影响以下现有规范：
- `external-rendering`: 默认渲染组合从 `HagicodeUpdateBulletin` 变更为 `HagicodeReleaseNotesMobile`，但 YAML 配置解析、CLI 渲染逻辑和组合覆盖机制保持不变

此变更不影响以下现有规范：
- `hagicode-update-bulletin`: 数据结构和验证逻辑保持不变，两个组件使用相同的 schema
