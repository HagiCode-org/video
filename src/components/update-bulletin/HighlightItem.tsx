...
export const HighlightItem: React.FC<HighlightItemProps> = ({
  ...
  // ========== CARD ENTRANCE ==========
  // Smooth entrance animation using spring
  const cardSpring = spring({
    frame: effectiveFrame,
    fps: config.fps,
    config: isMobile ? { damping: 200, stiffness: 100 } : { damping: 15, stiffness: 100, mass: 0.8 },
  });
  ...
  // Staggered entrance for title → description → screenshot for better readability
  const contentSpring = spring({
    frame: Math.max(0, effectiveFrame - 30), // 0.5 sec delay
    fps: config.fps,
    config: isMobile ? { damping: 180, stiffness: 80 } : { damping: 12, stiffness: 80 },
  });
  ...
  // Screenshot entrance with delay after description
  const screenshotSpring = spring({
    frame: Math.max(0, effectiveFrame - 60), // 1 sec delay
    fps: config.fps,
    config: isMobile ? { damping: 150, stiffness: 70 } : { damping: 10, stiffness: 70 },
  });

  const screenshotOpacity = interpolate(screenshotSpring, [0, 0.3, 1], [0, 0, 1]);
  ...
  // Timeline:
  //   0-240 frames (0-4 sec): Normal content display with screenshot in card
  //   240-270 frames (4-4.5 sec): Screenshot floats out from card
  //   270-360 frames (4.5-6 sec): Screenshot displayed fullscreen, card fades out
  //
  // IMPORTANT: Don't multiply by springMultiplier here, because effectiveFrame
  // is already the actual frame number (not scaled)
  const showcaseStartFrame = isMobile ? 240 : 180; // Fixed at 4 seconds for mobile (6 sec total)
